...
//  }
  ...
  public static float getFloat(Object value) {
    ...
    boolean isDp = false;//qlx fixed
    if (temp.endsWith("dp")) {
      temp = temp.substring(0, temp.indexOf("dp"));
      isDp = true;
    }
    ...
    if (isDp){
      int resutl_px = WXViewUtils.dip2px(result);
      return (float) ((resutl_px*1.0) / ((WXViewUtils.getScreenWidth() * 1.0) / WXEnvironment.sDefaultWidth));
    }
